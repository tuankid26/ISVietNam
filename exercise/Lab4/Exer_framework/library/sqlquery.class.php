...
class SQLQuery {
	...
	function search() {
		...
		if ($this->_result->num_rows > 0 ) {
				...
				if ($this->_hM == 1 && isset($this->hasMany)) {
					foreach ($this->hasMany as $aliasChild => $modelChild) {
						...
						if ($resultChild->num_rows > 0) {
							$numOfFieldsChild = $resultChild->field_count;
							for ($j = 0; $j < $numOfFieldsChild; ++$j) {
								array_push($tableChild, $resultChild->fetch_field_direct($j)->table);
								array_push($fieldChild, $resultChild->fetch_field_direct($j)->name);
							}

							while ($rowChild = $resultChild->fetch_row()) {
								for ($j = 0;$j < $numOfFieldsChild; ++$j) {
			...
			if ($this->_result->num_rows == 1 && $this->id != null) {
