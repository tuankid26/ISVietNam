...
class Filter extends Component {
    ...
    render() {
        ...
        const clause = district !== 'Tất cả' ? data.filter((p) => p.district === this.state.district) : data.filter((p) => p.province === this.state.province);
        return (
            <div className="filter">
                <label htmlFor="">Tỉnh/Thành phố</label>

                <select name={province} onChange={this.handleChangeProvince}>
                    {listProvince &&
                        listProvince.map((p) =>
                        ...
                        <div class="article__grid">
                            {
                                clause.map((r) => (
                                    ...
                                    <a href={"/place/".concat(r.ID_place)} class="article__item">
                                        ...
                                        <div class="article__image"><img src={'/images/Image/'.concat(this.state.img.filter((p) => p.ID_place === r.ID_place).map((x) => {return x.path})[0])} alt="" /></div>
                                        <div class="article__text">
                                            <div class="article__title">{r.name_place}
                                            </div>
